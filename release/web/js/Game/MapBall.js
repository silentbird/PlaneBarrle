...
var MapBall = /** @class */ (function (_super) {
        ...
        if (data) //如果是分裂的球的话 执行
         {
            ...
            if (r > 0.5) {
                ball.x = -30;
                if (type == 5) {
                    Laya.Tween.to(ball, { x: Laya.stage.width / 2 }, 800);
                }
                else {
                    Laya.Tween.to(ball, { x: Laya.stage.width / 2 - 80 }, 800);
                }
            }
            else {
    ...
    */
    ...
    MapBall.prototype.dropSpecialItem = function (data) {
        ...
        if (rrrrrr >= 0.85 && rrrrrr < 1 && !Launcher && !Invincible && !Laser && this.isdropItem() && !GlobleFun.haveProp && GlobleFun.bulletInterval < 120) { //散弹 15%（攻速达到120发/秒后不再掉落）
            Shotgun = true;
        }
        if (rrrrrr >= 0.70 && rrrrrr < 0.85 && !Shotgun && !Invincible && !Laser && this.isdropItem() && !GlobleFun.haveProp) { //发射器 15%
            Launcher = true;
        }
        if (rrrrrr >= 0.68 && rrrrrr < 0.70 && !Launcher && !Shotgun && !Laser && this.isdropItem() && !GlobleFun.haveProp) { //无敌保护 2%
            Invincible = true;
        }
        if (rrrrrr >= 0 && rrrrrr <= 0.13 && !Launcher && !Shotgun && !Invincible && this.isdropItem() && !GlobleFun.haveProp && GlobleFun.Score >= 200000) { //激光13%（当分数到达200000时有几率掉落）
            Laser = true;
        }
        if (Shotgun) {
